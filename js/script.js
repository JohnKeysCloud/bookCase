...
function book(title, author, pages, read, cover) {
  let newBook = Object.create(proto); // ? sets the pseudo proto of newBook to book.proto
...
function addEventListeners() {
  const readToggleBtns = document.querySelectorAll(
    '.book-card-btn:first-of-type'
  );
  const removeBookBtns = document.querySelectorAll(
    '.book-card-btn:nth-of-type(2)'
  );
  const editBookBtns = document.querySelectorAll(
    '.book-card-btn:nth-of-type(3)'
  );
...
function formatDate(dateAdded) {
  ...
  [dateAddedSplit[0], dateAddedSplit[1], dateAddedSplit[2]] = [
    dateAddedSplit[2],
    dateAddedSplit[0],
    dateAddedSplit[1],
  ];
...
function populateBookCase() {
  ...
  if (!sortSelect.value)
    bookCaseArray.sort((a, b) =>
      formatDate(a.dateAdded) > formatDate(b.dateAdded) ? -1 : 1
    );

  bookCaseArray.forEach((book) => {
    const bookCard = document.createElement('article');
    const bookImg = document.createElement('img');
    const bookImgDefaultSource =
      'assets/images/book-case/no-image-placeholder.svg';
...
function findBookObject() {
  let targetedBookTitle =
    this.parentNode.parentNode.querySelector('.bookcase-title').textContent;
...
function appendEmptyBookCaseContent() {
  ...
  bookCaseEmptyDivP.innerHTML =
    'your bookcase is empty<span id="bookcase-empty-periods">...</span> read a book ♡';
  bookCaseEmptyDivP2.classList.add('no-select');
  bookCaseEmptyDivP2.innerHTML =
    '<span class="add-symbol">⨭</span> Click to Add';
...
function editBookInfo() {
  const bookInfoPseudoInputs =
    this.parentNode.parentNode.querySelectorAll('.book-info');
  const targetedBookTitle =
    this.parentNode.parentNode.querySelector('.bookcase-title').textContent;
  const targetedBook = bookCaseArray.find(
    (book) => book.title === targetedBookTitle
  );

  if (this.textContent === 'Save Changes') {
    bookInfoPseudoInputs.forEach((info) =>
  ...
  for (let i = 0; i < bookInfoPseudoInputs.length; i++) {
    ...
    bookInfoPseudoInputs[i].addEventListener('input', () => {
      if (bookInfoPseudoInputs[i].classList.contains('bookcase-title')) {
        ...
        bookInfoPseudoInputs[i].classList.contains('bookcase-author-name')
      ) {
        targetedBook.author = bookInfoPseudoInputs[i].textContent;
      } else if (
        bookInfoPseudoInputs[i].classList.contains('bookcase-page-amount')
      ) {
...
function removeBookFromCase() {
  let targetedBookTitle =
    this.parentNode.parentNode.querySelector('.bookcase-title').textContent;
  let targetedBookObject = bookCaseArray.find(
    (book) => book.title === targetedBookTitle
  );
